...
class ModuleApplication(tk.Frame):
    ...
    def read_standard_data(self):
        response = self.interface.request(READ_DATA_REQUEST, max_attempts=5)
    ...
    def on_read_static_click(self):
        ...
        if not self.interface:
            ...
            return
        static_read_error = None
        try:
            response = self.interface.request(READ_MSG_CMD, max_attempts=5)
            ...
            static_read_error = e
            self.obi_instance.update_debug(f"Static battery frame read failed: {e}")
        ...
        if static_read_error:
            tk.messagebox.showerror("Error", f"Battery detected but protocol is not fully supported: {last_exception}")
        else:
    ...
    def on_read_data_click(self):
        ...
        try:
            if self.command_version == 'F0513':
                battery_data = self.read_f0513_data()
            else:
                try:
                    battery_data = self.read_standard_data()
                except Exception as standard_error:
                    self.obi_instance.update_debug(f"Standard protocol read failed, trying F0513 fallback: {standard_error}")
                    battery_data = self.read_f0513_data()
                    self.command_version = 'F0513'
                    messagebox.showwarning("Limited", "Switched to legacy diagnostics mode (F0513).")
    ...
    def on_reset_errors_click(self):
        ...
        reset_sequences = [
            ("standard", [TESTMODE_CMD, RESET_ERROR_CMD]),
            ("legacy", [F0513_TESTMODE_CMD, RESET_ERROR_CMD]),
            ("direct", [RESET_ERROR_CMD]),
        ]

        last_exception = None
        for sequence_name, commands in reset_sequences:
            try:
                self.obi_instance.update_debug(f"Trying reset sequence: {sequence_name}")

                for idx, command in enumerate(commands):
                    self.interface.request(command, max_attempts=5)

                    # Give the BMS a short settle time between test-mode and reset command.
                    if idx < len(commands) - 1:
                        time.sleep(0.05)

                self.obi_instance.update_debug(f"Reset sequence succeeded: {sequence_name}")
                messagebox.showinfo("Success", "Reset command sent successfully.")
                return

            except Exception as e:
                last_exception = e
                self.obi_instance.update_debug(f"Reset sequence failed ({sequence_name}): {e}")

        tk.messagebox.showerror("Error", f"Failed to reset errors: {last_exception}")
