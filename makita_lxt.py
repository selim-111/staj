...
TESTMODE_LEGACY_CMD = [0x01, 0x03, 0x00, 0x33, 0xD9, 0x96, 0xA5]
TESTMODE_CC_CMD     = [0x01, 0x03, 0x00, 0xCC, 0xD9, 0x96, 0xA5]
...
RESET_ERROR_LEGACY_CMD = [0x01, 0x02, 0x00, 0x33, 0xDA, 0x04]
RESET_ERROR_CC_CMD  = [0x01, 0x02, 0x00, 0xCC, 0xDA, 0x04]
...
class ModuleApplication(tk.Frame):
    ...
    def get_f0513_model(self):
        try:
            ...
            self.current_model = f"BL{response[2]:X}{response[3]:X}"
    ...
    def on_reset_errors_click(self):
        ...
        try:
            ...
            for sequence_name, testmode_cmd, reset_cmd in reset_sequences:
                try:
                    if self.obi_instance:
                        self.obi_instance.update_debug(f"Reset sequence: {sequence_name}")
                    self.interface.request(testmode_cmd)
                    time.sleep(0.05)
            ...
            raise Exception("No reset sequence was executed.")
