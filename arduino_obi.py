    ...
    def request(self, request, max_attempts=4):
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                # Older packs may respond slower on some frames.
                time.sleep(0.01)

                response = self.serial.read(expected_length)
                if len(response) != expected_length:
                    raise ValueError(f"Incomplete response ({len(response)}/{expected_length} bytes)")

                if request[2] == 0:
                    self.obi_instance.update_debug(f"<< {' '.join(f'{x:02X}' for x in response)}")
                    if response[0] != request[3] or response[1] != 0x00:
                        raise ValueError("Invalid response header for zero-length command")
                    return response
                ...
                if all(byte == 0xFF for byte in response_data):
                    raise ValueError("Invalid response: all bytes are 0xFF")

                if all(byte == 0x00 for byte in response_data):
                    raise ValueError("Invalid response: all bytes are 0x00")

                if payload and response_data.startswith(payload):
                    trailing = response_data[len(payload):]
                    if len(trailing) == 0 or all(byte in (0x00, 0xFF) for byte in trailing):
                        raise ValueError("Invalid response: echoed request pattern")
